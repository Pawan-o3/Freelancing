...
const ProjectData = () => {
  ...
  const fetchProject = async(id) =>{
    await axios.get(`${API_BASE_URL}/fetch-project/${id}`).then(
      (response)=>{
    ...
    const handleBidding = async() =>{
      ...
      await axios.post(`${API_BASE_URL}/make-bid`, {clientId, freelancerId, projectId, proposal, bidAmount, estimatedTime}).then(
        (response)=>{
            setProposal('');
            setBidAmount(0);
            setEstimatedTime(0);
    ...
    const handleProjectSubmission = async() =>{
      ...
      await axios.post(`${API_BASE_URL}/submit-project`, {clientId, freelancerId, projectId, projectLink, manualLink, submissionDescription}).then(
        (response)=>{
            setProjectLink('');
            setManualLink('');
    ...
    const fetchChats = async() =>{
      await axios.get(`${API_BASE_URL}/fetch-chats/${params['id']}`).then(
        (response) =>{
